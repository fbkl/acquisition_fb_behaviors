...
class CheckFileSavedState(EventState):
            ...
            try:
                ...
                for file in userdata.expected_files:
                    ...
                    for file_that_exist in files_in_dir:
                        Logger.logdebug(f"CheckFileSavedState comparing file: {file_that_exist} to file {file} with additional requirement of having {self._file} in its name!")
                ...
                if rospy.Time.now() - self._start_time > self._target_time:
                    Logger.loginfo(f"CheckFileSavedState files_found: {repr(files_found)}")
                    if len(files_found) == 0:
                        self._errors.append("no files found")
                        Logger.logerr("CheckFileSavedState NO FILE FOUND!!!!")
                    if len(files_missing) != 0:
                        Logger.logerr(f"CheckFileSavedState files_missing: {repr(files_missing)}")
                        self._errors.append(f"files_missing: {repr(files_missing)}")
                        self._done = 'failed'
                        return 'failed'
                    else:
                        self._done = 'continue'
                        Logger.loginfo(f"CheckFileSavedState All OK. Done.")
                        return 'continue'
                else:
                    if len(files_missing) == 0:
                        self._done = 'continue'
                ...
                Logger.logerr("CheckFileSavedState Something failed while checking for files!")
                traceback.print_exc()
    ...
    def on_enter(self, userdata):
        ...
        if time_to_wait > 0:
            Logger.loginfo('CheckFileSavedState Need to wait for %.1f seconds.' % time_to_wait)
