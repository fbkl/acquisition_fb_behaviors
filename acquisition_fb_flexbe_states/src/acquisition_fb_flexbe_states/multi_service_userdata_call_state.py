...
class MultiServiceUserdataCallState(EventState):
    '''
    ...
    def __init__(self, predicate,prefix,wait_to_start=False,timeout=60):
        # Declare outcomes, input_keys, and output_keys by calling the super constructor with the corresponding arguments.
        super(MultiServiceUserdataCallState, self).__init__(outcomes = ['done', 'failed'], input_keys=["multi_service_list"])
...
#		if rospy.Time.now() - self._start_time > self._multi_service_list:
    ...
    def on_enter(self, userdata):
        ...
        multi_service_list = userdata.multi_service_list
        ...
        self._multi_service_plex = multiservice_plex.MultiServiceCaller(self._multi_service_list, wait_to_start=self._wait_to_start, wait_for_responses = True, timeout=rospy.Duration(self._timeout))
